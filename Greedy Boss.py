...
def greedy_boss(days_in_simulation, bribe_cost_increment, plot_type = STANDARD):
    ...
    bribe_earning_boost = SALARY_INCREMENT
    
    # initialize list consisting of days vs. total salary earned for analysis
    days_vs_earnings = [(0, 0)]
    #print type(days_vs_earnings)

    # Each iteration of this while loop simulates one bribe
    while current_day <= days_in_simulation:
        
        # check whether we have enough savings to bribe without waiting
        if current_earnings < bribe_cost:
       
        # update state of simulation to reflect bribe
            days_to_bribe = math.ceil((float(bribe_cost - current_earnings)) / float(daily_salary))
            total_earnings += (daily_salary * days_to_bribe)
            current_earnings += (daily_salary * days_to_bribe)
            current_day += days_to_bribe
            #print "CD is", current_day
            #print "CE is", current_earnings
            if current_day > days_in_simulation:
                return days_vs_earnings
            
        # advance current_day to day of next bribe (DO NOT INCREMENT BY ONE DAY)
        
        #current_earnings += daily_salary
        #total_earnings += daily_salary
        
        
        current_earnings = current_earnings - bribe_cost
        bribe_cost += bribe_cost_increment
        daily_salary = daily_salary + bribe_earning_boost
